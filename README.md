...
The runner pins Harbor, the Python patch version used to run it, Harbor's resolved dependency constraints, the Codex agent runtime, model, supported low reasoning effort, attempts, concurrency, and retry count. It explicitly sets Docker's runtime platform to the current Docker server platform and records the host OS/architecture, Python platform, container platform, and resolved child-image manifest digest. It also preserves the exact evaluation commit in a Git bundle, scans every introduced Git object for high-confidence credentials, verifies the recorded constraints SHA-256 against the file inside that bundle, and emits sanitized judge timing/usage evidence. On a feature branch, the recorded merge base is the bundle's exact prerequisite; on the default branch, the bundle is self-contained and the base is recorded as `null`. This keeps provenance usable after a squash merge and permits the documented command from a clean `main` checkout without treating a compressed pack file as a secret scan. The current Luna route rejected `minimal`, so this smoke pins the lowest accepted value, `low`, and never switches models or reasoning levels automatically. The public scorecard also records the host `uv` version. The runner uses the existing Codex subscription session without copying credentials into the repository.
