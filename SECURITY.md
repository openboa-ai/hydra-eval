...
The smoke runner may use the existing Codex subscription session through Harbor's auth-file path and the local Codex CLI. Never pass the auth contents through an agent environment argument, print them, or copy them into `jobs/` or `results/`. The evidence collector allowlists selected output and rejects API keys, GitHub and Slack tokens, AWS access keys, OAuth token fields, and private-key headers before publication. Source bundles are inspected object by object, including blobs deleted before the recorded revision; scanning only the compressed bundle bytes is not a security check.
