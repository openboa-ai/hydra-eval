...
Model-graded assertions must preserve the rubric, reason, and grader name, version, and model. Human-reviewed assertions must preserve the rubric, reason, and reviewer name. Measures need concrete nonnegative numeric observations; nesting an opaque `score`, `overall_score`, or `composite_score` does not turn it into evidence.
